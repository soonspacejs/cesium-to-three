...
void c23_evaluateCircle(inout c23_surfaceEvaluation evaluation) {
	...
	// localMeters 的 x/y 分别表示东/北方向。标绘方位角约定为正北 0°、顺时针递增，
	// 因此必须使用 atan(东, 北)；atan(北, 东) 会退回数学角度，导致旋转后的扇形
	// 显示区域与按地理方位角生成的拾取代理错开。
	float circleHeading = c23_wrappedPositiveAngle(atan(circleVectorMeters.x, circleVectorMeters.y));
	float sectorStart = c23_wrappedPositiveAngle(c23_circleSectorStartRadians);
	float sectorLocalAngle = c23_wrappedPositiveAngle((circleHeading - sectorStart) * sectorDirection);
