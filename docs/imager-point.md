...
扩展现有 `point` 标绘，新增 `image` 样式。点击地图一次后，以点击经纬度为图片中心，将 `/xiaohuoshuan.png` 贴合地形或 3D Tiles 表面渲染，并自动结束本次拾取。
...
- 图片宽度沿东西方向、高度沿南北方向，中心锚定；通过 ENU 坐标旋转后生成四角，继续支持 `TERRAIN`、`CESIUM_3D_TILE` 和 `BOTH`。
...
- 验证图片尺寸为 10 × 11.416 米、中心落在点击点、顶部朝北、PNG 透明区域无底色。
