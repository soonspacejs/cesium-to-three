...
- [ ] 第一次单击已有文本只选中，不创建；
- [ ] 单击后马上看到位置、尺寸、旋转都正确的高亮；
- [ ] 选中后可直接拖动；
- [ ] 双击或 `F2` 可编辑文本；
- [ ] `Escape` 可取消编辑且不会卡住；
- [ ] 小棕色圆能稳定选中、拖动；
- [ ] 图片点、圆、扇形、箭头、线、面均只有可见形状内可选；
- [ ] 重叠对象按最近交点/视觉顺序选择；
- [ ] 创建工具不会吞掉已有实体的第一次点击；
- [ ] 反复创建、删除、撤销、重做后无幽灵命中；
- [ ] 关闭编辑器后无残留代理、DOM 输入框或相机锁。

## 通过标准

所有静态硬门槛、单元测试、交互集成测试和人工清单通过后，才允许删除旧 `FeatureHitTester` 的实体像素命中代码。任何一种图形仍依赖像素 fallback，都不算迁移完成。
