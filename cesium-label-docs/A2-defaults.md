# A2 · `text-defaults.ts` —— 选项解析与校验

> [← A1-types](./A1-types.md) | [A3-color →](./A3-color.md)

## 职责

`PlotTextOptions` → `ResolvedPlotTextOptions`，统一校验与默认值。所有非法输入 `throw new Error()`（与 `CesiumGroundRectanglePrimitive` 等一致）。`rotation` 度→弧度，`padding` 拆四元组，opacity 钳 [0,100]，`metersPerPixel` 取正。
...
锚点越界抛错；`content=''` / `fontSize<=0` / 颜色空串抛错；`textAlign='x' as any` → `'left'`；`padding=8` → `[8,8,8,8]`、负 padding 钳 0；`rotation=90` → `rotationRadians=π/2`；`fillOpacity=200` → 100、`NaN` → 100；`metersPerPixel=0` → 默认 1；不传 `boxWidth` → `null`、传 `-5` → `null`、传 `100` → `100`。
